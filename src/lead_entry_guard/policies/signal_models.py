...
Scope: 3B.0 (contract) + A3 (suspicious_domain) + A4 (source_conflict) + A6 (shared_inbox)
       + C1 (missing_context) + C2 (conflicting_context) + C3 (false_clarity)
...
class SignalDefinition(BaseModel):
    ...
    signal_family:
      "data"    — A-series: concrete observations (field values, source provenance).
      "context" — C-series: decision readiness conclusions.
      No default — omitting signal_family is a construction error.

    Validation enforced at construction time.
    """
    code: str
    signal_family: Literal["data", "context"]
...
class LeadSignalContext:
    ...
    Each rule takes what it needs:
      ...
      - C1:     use email + fields (identity anchor check)
      - C2:     use fields (routing-relevant conflict detection)
      - C3:     use email + fields (false clarity heuristics)
...
class DecisionResultV2:
    ...
    def has_context_signal(self) -> bool:
        """Convenience — True if any context-family signal was emitted."""
        return any(s.signal_family == "context" for s in self.signals)

    def has_data_signal(self) -> bool:
        """Convenience — True if any data-family signal was emitted."""
        return any(s.signal_family == "data" for s in self.signals)
